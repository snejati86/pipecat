...
//! Groq service implementation for the Pipecat Rust framework.
//!
//! This module provides:
//!
//! - [`GroqLLMService`] -- streaming chat-completion LLM service that talks to
//!   the Groq `/openai/v1/chat/completions` endpoint.
//!
//! Groq offers an OpenAI-compatible API with extremely fast inference on
//! models such as Llama 3.3 70B, Llama 3.1 8B, Mixtral 8x7B, and Gemma2 9B.
//! Because the wire format is identical to OpenAI's streaming chat completions,
//! this service mirrors the OpenAI implementation with adjusted defaults.
//!
//! # Supported models
//!
...
// ---------------------------------------------------------------------------
// Groq API request / response types (OpenAI-compatible subset for streaming)
// ---------------------------------------------------------------------------

/// Body sent to Groq's `/openai/v1/chat/completions`.
...
/// streaming inference call against the Groq API, emitting
...
///
/// ```ignore
/// use pipecat::services::groq::GroqLLMService;
///
/// let service = GroqLLMService::new("gsk_abc123", "")
...
impl GroqLLMService {
    ...
    /// Create a new `GroqLLMService`.
    ///
    /// # Arguments
    ///
    /// * `api_key` -- Groq API key (typically starts with `gsk_`).
    /// * `model` -- Model identifier (e.g. `"llama-3.3-70b-versatile"`). Pass
    ///   an empty string to use [`Self::DEFAULT_MODEL`].
    pub fn new(api_key: impl Into<String>, model: impl Into<String>) -> Self {
        ...
        Self {
            base: BaseProcessor::new(Some(format!("GroqLLMService({})", model)), false),
            api_key,
            model,
    ...
    async fn process_streaming_response(&mut self) {
        ...
        debug!(
            ...
            "Starting streaming chat completion via Groq"
        );
            ...
            .await
        {
            Ok(resp) => resp,
            Err(e) => {
                error!(error = %e, "Failed to send Groq chat completion request");
                let err_frame =
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %error_body, "Groq API returned an error");
            let err_frame = Arc::new(ErrorFrame::new(
                format!("Groq API error (HTTP {status}): {error_body}"),
                false,
            ));
            self.base
                .pending_frames
        ...
        while let Some(chunk_result) = byte_stream.next().await {
            let chunk = match chunk_result {
                Ok(c) => c,
                Err(e) => {
                    error!(error = %e, "Error reading Groq SSE stream");
                    let err_frame = Arc::new(ErrorFrame::new(
            ...
            while let Some(newline_pos) = line_buffer.find('\n') {
                ...
                if data == "[DONE]" {
                    debug!("Groq SSE stream completed");
                    break;
                }
                ...
                let chunk: ChatCompletionChunk = match serde_json::from_str(data) {
                    Ok(c) => c,
                    Err(e) => {
                        warn!(error = %e, data = %data, "Failed to parse Groq SSE chunk JSON");
                        continue;
                    }
                };
...
impl fmt::Debug for GroqLLMService {
...
impl_base_display!(GroqLLMService);
...
#[async_trait]
impl FrameProcessor for GroqLLMService {
    ...
    async fn process_frame(&mut self, frame: Arc<dyn Frame>, direction: FrameDirection) {
        ...
        if let Some(append) = frame.as_any().downcast_ref::<LLMMessagesAppendFrame>() {
            ...
            debug!(
                ...
                "Appended messages, starting Groq inference"
            );
        ...
        if let Some(result_frame) = frame.as_any().downcast_ref::<FunctionCallResultFrame>() {
            ...
            debug!(
                ...
                "Function call result received, re-running Groq inference"
            );
            self.process_streaming_response().await;
            return;
        }
...
#[async_trait]
impl AIService for GroqLLMService {
    fn model(&self) -> Option<&str> {
        Some(&self.model)
    }

    async fn start(&mut self) {
        debug!(model = %self.model, "GroqLLMService started");
    }

    async fn stop(&mut self) {
        debug!("GroqLLMService stopped");
        self.messages.clear();
    }

    async fn cancel(&mut self) {
        debug!("GroqLLMService cancelled");
    }
}
...
impl LLMService for GroqLLMService {
    ...
    async fn run_inference(&mut self, messages: &[serde_json::Value]) -> Option<String> {
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %body_text, "Groq run_inference API error");
            return None;
        }
        ...
        let parsed: ChatCompletionResponse = match response.json().await {
            Ok(p) => p,
            Err(e) => {
                error!(error = %e, "Failed to parse Groq run_inference response");
                return None;
            }
        };
...
mod tests {
    ...
    fn test_new_with_default_model() {
        let svc = GroqLLMService::new("gsk_test_key", "");
        assert_eq!(svc.model, GroqLLMService::DEFAULT_MODEL);
        assert_eq!(svc.api_key, "gsk_test_key");
        assert_eq!(svc.base_url, GroqLLMService::DEFAULT_BASE_URL);
    }

    #[test]
    fn test_new_with_custom_model() {
        let svc = GroqLLMService::new("gsk_key", "llama-3.1-8b-instant");
        assert_eq!(svc.model, "llama-3.1-8b-instant");
    }

    #[test]
    fn test_with_model_builder() {
        let svc = GroqLLMService::new("gsk_key", "").with_model("mixtral-8x7b-32768");
        assert_eq!(svc.model, "mixtral-8x7b-32768");
    }

    #[test]
    fn test_with_base_url_builder() {
        let svc =
            GroqLLMService::new("gsk_key", "").with_base_url("https://custom-proxy.example.com");
        assert_eq!(svc.base_url, "https://custom-proxy.example.com");
    }

    #[test]
    fn test_with_temperature_builder() {
        let svc = GroqLLMService::new("gsk_key", "").with_temperature(0.7);
        assert_eq!(svc.temperature, Some(0.7));
    }

    #[test]
    fn test_with_max_tokens_builder() {
        let svc = GroqLLMService::new("gsk_key", "").with_max_tokens(2048);
        assert_eq!(svc.max_tokens, Some(2048));
    }

    #[test]
    fn test_builder_chaining() {
        let svc = GroqLLMService::new("gsk_key", "")
            .with_model("gemma2-9b-it")
            .with_temperature(0.5)
            .with_max_tokens(512)
            .with_base_url("https://proxy.test");
        assert_eq!(svc.model, "gemma2-9b-it");
    ...
    fn test_default_base_url_constant() {
        assert_eq!(
            GroqLLMService::DEFAULT_BASE_URL,
    ...
    fn test_default_model_constant() {
        assert_eq!(GroqLLMService::DEFAULT_MODEL, "llama-3.3-70b-versatile");
    }
    ...
    fn test_build_request_basic() {
        let svc = GroqLLMService::new("gsk_key", "llama-3.3-70b-versatile");
    ...
    fn test_build_request_with_temperature() {
        let svc = GroqLLMService::new("gsk_key", "").with_temperature(0.3);
        let req = svc.build_request();
    ...
    fn test_build_request_with_max_tokens() {
        let svc = GroqLLMService::new("gsk_key", "").with_max_tokens(4096);
        let req = svc.build_request();
    ...
    fn test_build_request_with_messages() {
        let mut svc = GroqLLMService::new("gsk_key", "");
    ...
    fn test_parse_content_chunk() {
        let json = r#"{"id":"chatcmpl-123","model":"llama-3.3-70b-versatile","choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}]}"#;
    ...
    fn test_parse_chunk_with_model_field() {
        let json = r#"{"id":"chatcmpl-model","model":"llama-3.1-8b-instant","choices":[{"index":0,"delta":{"content":"hi"},"finish_reason":null}]}"#;
        let chunk: ChatCompletionChunk = serde_json::from_str(json).expect("parse should succeed");
        assert_eq!(chunk.model.as_deref(), Some("llama-3.1-8b-instant"));
    }
    ...
    fn test_usage_metrics_data_construction() {
        let usage_metrics = LLMUsageMetricsData {
            processor: "GroqLLMService(llama-3.3-70b-versatile)".to_string(),
    ...
    fn test_model_mixtral() {
        let svc = GroqLLMService::new("gsk_key", "mixtral-8x7b-32768");
        assert_eq!(svc.model, "mixtral-8x7b-32768");
    }

    #[test]
    fn test_model_gemma2() {
        let svc = GroqLLMService::new("gsk_key", "gemma2-9b-it");
        assert_eq!(svc.model, "gemma2-9b-it");
    }
    ...
    fn test_debug_format() {
        ...
        assert!(debug_str.contains("GroqLLMService"));
        assert!(debug_str.contains("llama-3.3-70b-versatile"));
        assert!(debug_str.contains("api.groq.com"));
    }
    ...
    fn test_display_format() {
        ...
        assert!(display_str.contains("GroqLLMService"));
    }
    ...
    fn test_model_trait_method() {
        let svc = GroqLLMService::new("gsk_key", "mixtral-8x7b-32768");
        assert_eq!(svc.model(), Some("mixtral-8x7b-32768"));
    }
    ...
    fn test_base_accessor() {
        let svc = GroqLLMService::new("gsk_key", "");
        let base = svc.base();
        assert!(base.name().contains("GroqLLMService"));
    }
    ...
    fn test_streaming_url_construction() {
        ...
        assert_eq!(expected, "https://api.groq.com/openai/v1/chat/completions");

        // With custom base URL
        let svc2 = GroqLLMService::new("gsk_key", "").with_base_url("https://proxy.test/api");
    ...
    fn test_very_long_model_name() {
        ...
        let svc = GroqLLMService::new("gsk_key", long_name.clone());
        assert_eq!(svc.model, long_name);
    }

    #[test]
    fn test_temperature_boundary_zero() {
        let svc = GroqLLMService::new("gsk_key", "").with_temperature(0.0);
        assert_eq!(svc.temperature, Some(0.0));
    }

    #[test]
    fn test_temperature_boundary_two() {
        let svc = GroqLLMService::new("gsk_key", "").with_temperature(2.0);
        assert_eq!(svc.temperature, Some(2.0));
    }

    #[test]
    fn test_max_tokens_boundary_one() {
        let svc = GroqLLMService::new("gsk_key", "").with_max_tokens(1);
        assert_eq!(svc.max_tokens, Some(1));
    }

    #[test]
    fn test_max_tokens_large_value() {
        let svc = GroqLLMService::new("gsk_key", "").with_max_tokens(131072);
    ...
    fn test_parse_chunk_extra_fields_ignored() {
        // Groq may return fields not in our struct; serde should ignore them.
