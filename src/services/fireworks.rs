...
//! Fireworks LLM service for the Pipecat Rust framework.
//!
//! [`FireworksLLMService`] is a type alias for [`GenericLlmService`] with
//! [`FireworksProtocol`], providing streaming chat completions via the
//! Fireworks API.

use crate::services::shared::generic_llm::GenericLlmService;
use crate::services::shared::llm_protocol::LlmProtocol;

/// Protocol configuration for the Fireworks API.
#[derive(Debug, Clone, Default)]
pub struct FireworksProtocol;

impl LlmProtocol for FireworksProtocol {
...
/// Fireworks chat-completion LLM service with streaming SSE support.
pub type FireworksLLMService = GenericLlmService<FireworksProtocol>;
...
mod tests {
    ...
    fn test_service_custom_model() {
        let svc = FireworksLLMService::new("test-key", "custom-model");
        assert_eq!(svc.model, "custom-model");
    }

    #[test]
    fn test_service_builder() {
        let svc = FireworksLLMService::new("test-key", "model")
    ...
    fn test_display_and_debug() {
        ...
        assert!(display.contains("Fireworks"));
        let debug = format!("{:?}", svc);
        assert!(debug.contains("Fireworks"));
    }
}
