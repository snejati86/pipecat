...
//! SambaNova service implementation for the Pipecat Rust framework.
//!
//! This module provides:
//!
//! - [`SambaNovaLLMService`] -- streaming chat-completion LLM service that
//!   talks to the SambaNova `/v1/chat/completions` endpoint.
//!
//! SambaNova is an enterprise inference platform with an OpenAI-compatible API
//! that provides extremely fast inference on models such as Meta-Llama 3.1 and
//! 3.3 variants. Because the wire format is identical to OpenAI's streaming
//! chat completions, this service mirrors the OpenAI implementation with
//! adjusted defaults.
//!
//! # Supported models
//!
...
// ---------------------------------------------------------------------------
// SambaNova API request / response types (OpenAI-compatible subset for streaming)
// ---------------------------------------------------------------------------

/// Body sent to SambaNova's `/v1/chat/completions`.
...
/// SambaNova chat-completion LLM service with streaming SSE support.
///
...
/// streaming inference call against the SambaNova API, emitting
...
///
/// ```ignore
/// use pipecat::services::sambanova::SambaNovaLLMService;
///
/// let service = SambaNovaLLMService::new("sn_api_key_123", "")
///     .with_temperature(0.7)
///     .with_max_tokens(1024)
///     .with_top_p(0.9)
///     .with_top_k(50);
/// ```
pub struct SambaNovaLLMService {
    base: BaseProcessor,
    api_key: String,
    model: String,
    base_url: String,
    ...
    top_p: Option<f64>,
    /// Optional top_k (top-k sampling) override.
    top_k: Option<u64>,
}

impl SambaNovaLLMService {
    /// Default model used when none is specified.
    pub const DEFAULT_MODEL: &'static str = "Meta-Llama-3.1-70B-Instruct";

    /// Default base URL for the SambaNova API.
    pub const DEFAULT_BASE_URL: &'static str = "https://api.sambanova.ai/v1";

    /// Create a new `SambaNovaLLMService`.
    ///
    /// # Arguments
    ///
    /// * `api_key` -- SambaNova API key.
    /// * `model` -- Model identifier (e.g. `"Meta-Llama-3.1-70B-Instruct"`).
    ///   Pass an empty string to use [`Self::DEFAULT_MODEL`].
    pub fn new(api_key: impl Into<String>, model: impl Into<String>) -> Self {
        ...
        Self {
            base: BaseProcessor::new(Some(format!("SambaNovaLLMService({})", model)), false),
            api_key,
            model,
    ...
    async fn process_streaming_response(&mut self) {
        ...
        debug!(
            ...
            "Starting streaming chat completion via SambaNova"
        );
            ...
            .await
        {
            Ok(resp) => resp,
            Err(e) => {
                error!(error = %e, "Failed to send SambaNova chat completion request");
                let err_frame =
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %error_body, "SambaNova API returned an error");
            let err_frame = Arc::new(ErrorFrame::new(
                format!("SambaNova API error (HTTP {status}): {error_body}"),
                false,
            ));
            self.base
                .pending_frames
        ...
        // --- Parse SSE stream ----------------------------------------------------
        //
        // SambaNova uses the same SSE format as OpenAI:
        //
        //   data: {"id":"...","choices":[...]}\n\n
        //
        ...
        while let Some(chunk_result) = byte_stream.next().await {
            let chunk = match chunk_result {
                Ok(c) => c,
                Err(e) => {
                    error!(error = %e, "Error reading SambaNova SSE stream");
                    let err_frame = Arc::new(ErrorFrame::new(
            ...
            while let Some(newline_pos) = line_buffer.find('\n') {
                ...
                if data == "[DONE]" {
                    debug!("SambaNova SSE stream completed");
                    break;
                }
                ...
                let chunk: ChatCompletionChunk = match serde_json::from_str(data) {
                    Ok(c) => c,
                    Err(e) => {
                        warn!(error = %e, data = %data, "Failed to parse SambaNova SSE chunk JSON");
                        continue;
                    }
                };
...
impl fmt::Debug for SambaNovaLLMService {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("SambaNovaLLMService")
...
impl_base_display!(SambaNovaLLMService);
...
#[async_trait]
impl FrameProcessor for SambaNovaLLMService {
    ...
    async fn process_frame(&mut self, frame: Arc<dyn Frame>, direction: FrameDirection) {
        ...
        if let Some(append) = frame.as_any().downcast_ref::<LLMMessagesAppendFrame>() {
            ...
            debug!(
                ...
                "Appended messages, starting SambaNova inference"
            );
        ...
        if let Some(result_frame) = frame.as_any().downcast_ref::<FunctionCallResultFrame>() {
            ...
            debug!(
                ...
                "Function call result received, re-running SambaNova inference"
            );
            self.process_streaming_response().await;
            return;
        }
...
#[async_trait]
impl AIService for SambaNovaLLMService {
    fn model(&self) -> Option<&str> {
        Some(&self.model)
    }

    async fn start(&mut self) {
        debug!(model = %self.model, "SambaNovaLLMService started");
    }

    async fn stop(&mut self) {
        debug!("SambaNovaLLMService stopped");
        self.messages.clear();
    }

    async fn cancel(&mut self) {
        debug!("SambaNovaLLMService cancelled");
    }
}
...
#[async_trait]
impl LLMService for SambaNovaLLMService {
    ...
    async fn run_inference(&mut self, messages: &[serde_json::Value]) -> Option<String> {
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %body_text, "SambaNova run_inference API error");
            return None;
        }
        ...
        let parsed: ChatCompletionResponse = match response.json().await {
            Ok(p) => p,
            Err(e) => {
                error!(error = %e, "Failed to parse SambaNova run_inference response");
                return None;
            }
        };
...
mod tests {
    ...
    fn test_new_with_default_model() {
        let svc = SambaNovaLLMService::new("sn_test_key", "");
        assert_eq!(svc.model, SambaNovaLLMService::DEFAULT_MODEL);
        assert_eq!(svc.api_key, "sn_test_key");
        assert_eq!(svc.base_url, SambaNovaLLMService::DEFAULT_BASE_URL);
    }

    #[test]
    fn test_new_with_custom_model() {
        let svc = SambaNovaLLMService::new("sn_key", "Meta-Llama-3.1-8B-Instruct");
        assert_eq!(svc.model, "Meta-Llama-3.1-8B-Instruct");
    }

    #[test]
    fn test_with_model_builder() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_model("Meta-Llama-3.3-70B-Instruct");
        assert_eq!(svc.model, "Meta-Llama-3.3-70B-Instruct");
    }

    #[test]
    fn test_with_base_url_builder() {
        let svc = SambaNovaLLMService::new("sn_key", "")
    ...
    fn test_with_temperature_builder() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_temperature(0.7);
        assert_eq!(svc.temperature, Some(0.7));
    }

    #[test]
    fn test_with_max_tokens_builder() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_max_tokens(2048);
        assert_eq!(svc.max_tokens, Some(2048));
    }

    #[test]
    fn test_with_top_p_builder() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_p(0.9);
        assert_eq!(svc.top_p, Some(0.9));
    }

    #[test]
    fn test_with_top_k_builder() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_k(50);
    ...
    fn test_builder_chaining() {
        let svc = SambaNovaLLMService::new("sn_key", "")
            .with_model("Meta-Llama-3.1-405B-Instruct")
            .with_temperature(0.5)
            .with_max_tokens(512)
            .with_top_p(0.95)
            .with_top_k(40)
            .with_base_url("https://proxy.test");
        assert_eq!(svc.model, "Meta-Llama-3.1-405B-Instruct");
    ...
    fn test_default_base_url_constant() {
        assert_eq!(
            SambaNovaLLMService::DEFAULT_BASE_URL,
    ...
    fn test_default_model_constant() {
        assert_eq!(
            SambaNovaLLMService::DEFAULT_MODEL,
    ...
    fn test_build_request_basic() {
        let svc = SambaNovaLLMService::new("sn_key", "Meta-Llama-3.1-70B-Instruct");
        let req = svc.build_request();
        assert_eq!(req.model, "Meta-Llama-3.1-70B-Instruct");
        assert!(req.stream);
    ...
    fn test_build_request_with_temperature() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_temperature(0.3);
        let req = svc.build_request();
    ...
    fn test_build_request_with_max_tokens() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_max_tokens(4096);
        let req = svc.build_request();
    ...
    fn test_build_request_with_top_p() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_p(0.85);
        let req = svc.build_request();
    ...
    fn test_build_request_with_top_k() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_k(100);
        let req = svc.build_request();
    ...
    fn test_build_request_with_messages() {
        let mut svc = SambaNovaLLMService::new("sn_key", "");
    ...
    fn test_build_request_serialization() {
        ...
        assert!(json.contains("\"model\":\"Meta-Llama-3.1-70B-Instruct\""));
    ...
    #[test]
    fn test_build_request_serialization_with_top_p_and_top_k() {
        let svc = SambaNovaLLMService::new("sn_key", "")
            .with_top_p(0.9)
            .with_top_k(50);
        let req = svc.build_request();
    ...
    fn test_parse_content_chunk() {
        let json = r#"{"id":"chatcmpl-123","model":"Meta-Llama-3.1-70B-Instruct","choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}]}"#;
    ...
    fn test_parse_chunk_with_model_field() {
        let json = r#"{"id":"chatcmpl-model","model":"Meta-Llama-3.1-8B-Instruct","choices":[{"index":0,"delta":{"content":"hi"},"finish_reason":null}]}"#;
        let chunk: ChatCompletionChunk = serde_json::from_str(json).expect("parse should succeed");
        assert_eq!(chunk.model.as_deref(), Some("Meta-Llama-3.1-8B-Instruct"));
    }
    ...
    fn test_usage_metrics_data_construction() {
        let usage_metrics = LLMUsageMetricsData {
            processor: "SambaNovaLLMService(Meta-Llama-3.1-70B-Instruct)".to_string(),
    ...
    fn test_parse_non_streaming_response() {
        let json = r#"{
            "choices":[{
                "message":{"content":"Hello from SambaNova!"}
            }],
    ...
    fn test_model_llama_3_1_70b() {
        let svc = SambaNovaLLMService::new("sn_key", "Meta-Llama-3.1-70B-Instruct");
        assert_eq!(svc.model, "Meta-Llama-3.1-70B-Instruct");
        let req = svc.build_request();
        assert_eq!(req.model, "Meta-Llama-3.1-70B-Instruct");
    }
    ...
    #[test]
    fn test_model_llama_3_1_405b() {
        let svc = SambaNovaLLMService::new("sn_key", "Meta-Llama-3.1-405B-Instruct");
        assert_eq!(svc.model, "Meta-Llama-3.1-405B-Instruct");
    }

    #[test]
    fn test_model_llama_3_3_70b() {
        let svc = SambaNovaLLMService::new("sn_key", "Meta-Llama-3.3-70B-Instruct");
        assert_eq!(svc.model, "Meta-Llama-3.3-70B-Instruct");
    }
    ...
    fn test_debug_format() {
        ...
        assert!(debug_str.contains("SambaNovaLLMService"));
        assert!(debug_str.contains("Meta-Llama-3.1-70B-Instruct"));
        assert!(debug_str.contains("api.sambanova.ai"));
    }
    ...
    fn test_display_format() {
        ...
        assert!(display_str.contains("SambaNovaLLMService"));
    }
    ...
    fn test_model_trait_method() {
        let svc = SambaNovaLLMService::new("sn_key", "Meta-Llama-3.3-70B-Instruct");
        assert_eq!(svc.model(), Some("Meta-Llama-3.3-70B-Instruct"));
    }
    ...
    fn test_base_accessor() {
        let svc = SambaNovaLLMService::new("sn_key", "");
        let base = svc.base();
        assert!(base.name().contains("SambaNovaLLMService"));
    }
    ...
    fn test_streaming_url_construction() {
        ...
        assert_eq!(expected, "https://api.sambanova.ai/v1/chat/completions");

        // With custom base URL
        let svc2 = SambaNovaLLMService::new("sn_key", "").with_base_url("https://proxy.test/api");
    ...
    fn test_non_streaming_url_construction() {
        ...
        assert_eq!(url, "https://api.sambanova.ai/v1/chat/completions");
    }
    ...
    fn test_empty_api_key() {
        let svc = SambaNovaLLMService::new("", "");
    ...
    fn test_very_long_model_name() {
        ...
        let svc = SambaNovaLLMService::new("sn_key", long_name.clone());
        assert_eq!(svc.model, long_name);
    }

    #[test]
    fn test_temperature_boundary_zero() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_temperature(0.0);
        assert_eq!(svc.temperature, Some(0.0));
    }

    #[test]
    fn test_temperature_boundary_two() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_temperature(2.0);
        assert_eq!(svc.temperature, Some(2.0));
    }

    #[test]
    fn test_max_tokens_boundary_one() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_max_tokens(1);
        assert_eq!(svc.max_tokens, Some(1));
    }

    #[test]
    fn test_max_tokens_large_value() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_max_tokens(131072);
        assert_eq!(svc.max_tokens, Some(131072));
    }

    #[test]
    fn test_top_p_boundary_zero() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_p(0.0);
        assert_eq!(svc.top_p, Some(0.0));
    }

    #[test]
    fn test_top_p_boundary_one() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_p(1.0);
        assert_eq!(svc.top_p, Some(1.0));
    }

    #[test]
    fn test_top_k_boundary_one() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_k(1);
        assert_eq!(svc.top_k, Some(1));
    }

    #[test]
    fn test_top_k_large_value() {
        let svc = SambaNovaLLMService::new("sn_key", "").with_top_k(1000);
    ...
    fn test_parse_chunk_extra_fields_ignored() {
        // SambaNova may return fields not in our struct; serde should ignore them.
