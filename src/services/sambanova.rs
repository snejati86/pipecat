...
//! SambaNova LLM service for the Pipecat Rust framework.
//!
//! [`SambaNovaLLMService`] is a type alias for [`GenericLlmService`] with
//! [`SambaNovaProtocol`], providing streaming chat completions via the
//! SambaNova API.

use crate::services::shared::generic_llm::GenericLlmService;
use crate::services::shared::llm_protocol::LlmProtocol;

/// Protocol configuration for the SambaNova API.
#[derive(Debug, Clone, Default)]
pub struct SambaNovaProtocol;

impl LlmProtocol for SambaNovaProtocol {
...
pub type SambaNovaLLMService = GenericLlmService<SambaNovaProtocol>;
...
mod tests {
    ...
    fn test_service_creation() {
        let svc = SambaNovaLLMService::new("test-key", "");
    ...
    fn test_service_custom_model() {
        let svc = SambaNovaLLMService::new("test-key", "custom-model");
        assert_eq!(svc.model, "custom-model");
    }

    #[test]
    fn test_service_builder() {
        let svc = SambaNovaLLMService::new("test-key", "model")
    ...
    fn test_display_and_debug() {
        ...
        assert!(display.contains("SambaNova"));
        let debug = format!("{:?}", svc);
        assert!(debug.contains("SambaNova"));
    }
}
