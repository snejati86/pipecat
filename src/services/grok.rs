...
//! Grok (xAI) service implementation for the Pipecat Rust framework.
//!
//! This module provides:
//!
//! - [`GrokLLMService`] -- streaming chat-completion LLM service that talks to
//!   the xAI `/v1/chat/completions` endpoint.
//!
//! Grok is xAI's large language model with an OpenAI-compatible API. Because
//! the wire format is identical to OpenAI's streaming chat completions, this
//! service mirrors the OpenAI implementation with adjusted defaults.
//!
//! # Supported models
//!
//! - `grok-2` (default)
...
// ---------------------------------------------------------------------------
// Grok API request / response types (OpenAI-compatible subset for streaming)
// ---------------------------------------------------------------------------

/// Body sent to xAI's `/v1/chat/completions`.
...
/// Grok (xAI) chat-completion LLM service with streaming SSE support.
///
...
/// streaming inference call against the xAI API, emitting
...
///
/// ```ignore
/// use pipecat::services::grok::GrokLLMService;
///
/// let service = GrokLLMService::new("xai-abc123", "")
///     .with_temperature(0.7)
///     .with_max_tokens(1024);
/// ```
pub struct GrokLLMService {
    base: BaseProcessor,
    api_key: String,
    model: String,
    base_url: String,
    ...
    top_p: Option<f64>,
    /// Optional frequency_penalty override.
    frequency_penalty: Option<f64>,
    /// Optional presence_penalty override.
    presence_penalty: Option<f64>,
}

impl GrokLLMService {
    /// Default model used when none is specified.
    pub const DEFAULT_MODEL: &'static str = "grok-2";

    /// Default base URL for the xAI API.
    pub const DEFAULT_BASE_URL: &'static str = "https://api.x.ai/v1";

    /// Create a new `GrokLLMService`.
    ///
    /// # Arguments
    ///
    /// * `api_key` -- xAI API key.
    /// * `model` -- Model identifier (e.g. `"grok-2"`). Pass an empty string
    ...
    pub fn new(api_key: impl Into<String>, model: impl Into<String>) -> Self {
        ...
        Self {
            base: BaseProcessor::new(Some(format!("GrokLLMService({})", model)), false),
            api_key,
            model,
    ...
    /// Builder method: set the frequency penalty.
    pub fn with_frequency_penalty(mut self, frequency_penalty: f64) -> Self {
        self.frequency_penalty = Some(frequency_penalty);
        self
    }

    /// Builder method: set the presence penalty.
    ...
    async fn process_streaming_response(&mut self) {
        ...
        debug!(
            ...
            "Starting streaming chat completion via Grok (xAI)"
        );
            ...
            .await
        {
            Ok(resp) => resp,
            Err(e) => {
                error!(error = %e, "Failed to send Grok chat completion request");
                let err_frame =
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %error_body, "Grok API returned an error");
            let err_frame = Arc::new(ErrorFrame::new(
                format!("Grok API error (HTTP {status}): {error_body}"),
                false,
            ));
            self.base
                .pending_frames
        ...
        // --- Parse SSE stream ----------------------------------------------------
        //
        // Grok uses the same SSE format as OpenAI:
        //
        //   data: {"id":"...","choices":[...]}\n\n
        //
        ...
        while let Some(chunk_result) = byte_stream.next().await {
            let chunk = match chunk_result {
                Ok(c) => c,
                Err(e) => {
                    error!(error = %e, "Error reading Grok SSE stream");
                    let err_frame = Arc::new(ErrorFrame::new(
            ...
            while let Some(newline_pos) = line_buffer.find('\n') {
                ...
                if data == "[DONE]" {
                    debug!("Grok SSE stream completed");
                    break;
                }
                ...
                let chunk: ChatCompletionChunk = match serde_json::from_str(data) {
                    Ok(c) => c,
                    Err(e) => {
                        warn!(error = %e, data = %data, "Failed to parse Grok SSE chunk JSON");
                        continue;
                    }
                };
...
impl fmt::Debug for GrokLLMService {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GrokLLMService")
...
impl_base_display!(GrokLLMService);
...
#[async_trait]
impl FrameProcessor for GrokLLMService {
    ...
    async fn process_frame(&mut self, frame: Arc<dyn Frame>, direction: FrameDirection) {
        ...
        if let Some(append) = frame.as_any().downcast_ref::<LLMMessagesAppendFrame>() {
            ...
            debug!(
                ...
                "Appended messages, starting Grok inference"
            );
        ...
        if let Some(result_frame) = frame.as_any().downcast_ref::<FunctionCallResultFrame>() {
            ...
            debug!(
                ...
                "Function call result received, re-running Grok inference"
            );
            self.process_streaming_response().await;
            return;
        }
...
#[async_trait]
impl AIService for GrokLLMService {
    fn model(&self) -> Option<&str> {
        Some(&self.model)
    }

    async fn start(&mut self) {
        debug!(model = %self.model, "GrokLLMService started");
    }

    async fn stop(&mut self) {
        debug!("GrokLLMService stopped");
        self.messages.clear();
    }

    async fn cancel(&mut self) {
        debug!("GrokLLMService cancelled");
    }
}
...
#[async_trait]
impl LLMService for GrokLLMService {
    ...
    async fn run_inference(&mut self, messages: &[serde_json::Value]) -> Option<String> {
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %body_text, "Grok run_inference API error");
            return None;
        }
        ...
        let parsed: ChatCompletionResponse = match response.json().await {
            Ok(p) => p,
            Err(e) => {
                error!(error = %e, "Failed to parse Grok run_inference response");
                return None;
            }
        };
...
mod tests {
    ...
    fn test_new_with_default_model() {
        let svc = GrokLLMService::new("xai_test_key", "");
        assert_eq!(svc.model, GrokLLMService::DEFAULT_MODEL);
        assert_eq!(svc.api_key, "xai_test_key");
        assert_eq!(svc.base_url, GrokLLMService::DEFAULT_BASE_URL);
    }

    #[test]
    fn test_new_with_custom_model() {
        let svc = GrokLLMService::new("xai_key", "grok-2-mini");
        assert_eq!(svc.model, "grok-2-mini");
    }

    #[test]
    fn test_with_model_builder() {
        let svc = GrokLLMService::new("xai_key", "").with_model("grok-beta");
        assert_eq!(svc.model, "grok-beta");
    }

    #[test]
    fn test_with_base_url_builder() {
        let svc =
            GrokLLMService::new("xai_key", "").with_base_url("https://custom-proxy.example.com");
        assert_eq!(svc.base_url, "https://custom-proxy.example.com");
    }

    #[test]
    fn test_with_temperature_builder() {
        let svc = GrokLLMService::new("xai_key", "").with_temperature(0.7);
        assert_eq!(svc.temperature, Some(0.7));
    }

    #[test]
    fn test_with_max_tokens_builder() {
        let svc = GrokLLMService::new("xai_key", "").with_max_tokens(2048);
        assert_eq!(svc.max_tokens, Some(2048));
    }

    #[test]
    fn test_with_top_p_builder() {
        let svc = GrokLLMService::new("xai_key", "").with_top_p(0.9);
        assert_eq!(svc.top_p, Some(0.9));
    }

    #[test]
    fn test_with_frequency_penalty_builder() {
        let svc = GrokLLMService::new("xai_key", "").with_frequency_penalty(0.5);
        assert_eq!(svc.frequency_penalty, Some(0.5));
    }

    #[test]
    fn test_with_presence_penalty_builder() {
        let svc = GrokLLMService::new("xai_key", "").with_presence_penalty(0.3);
        assert_eq!(svc.presence_penalty, Some(0.3));
    }

    #[test]
    fn test_builder_chaining() {
        let svc = GrokLLMService::new("xai_key", "")
            .with_model("grok-2-mini")
        ...
        assert_eq!(svc.frequency_penalty, Some(0.1));
        assert_eq!(svc.presence_penalty, Some(0.2));
    ...
    fn test_default_frequency_penalty_is_none() {
        let svc = GrokLLMService::new("xai_key", "");
        assert!(svc.frequency_penalty.is_none());
    }

    #[test]
    fn test_default_presence_penalty_is_none() {
        let svc = GrokLLMService::new("xai_key", "");
        assert!(svc.presence_penalty.is_none());
    }
    ...
    fn test_default_base_url_constant() {
        assert_eq!(GrokLLMService::DEFAULT_BASE_URL, "https://api.x.ai/v1");
    }

    #[test]
    fn test_default_model_constant() {
        assert_eq!(GrokLLMService::DEFAULT_MODEL, "grok-2");
    }
    ...
    fn test_build_request_basic() {
        let svc = GrokLLMService::new("xai_key", "grok-2");
        let req = svc.build_request();
        assert_eq!(req.model, "grok-2");
        assert!(req.stream);
    ...
    fn test_build_request_with_temperature() {
        let svc = GrokLLMService::new("xai_key", "").with_temperature(0.3);
        let req = svc.build_request();
    ...
    fn test_build_request_with_max_tokens() {
        let svc = GrokLLMService::new("xai_key", "").with_max_tokens(4096);
        let req = svc.build_request();
    ...
    fn test_build_request_with_top_p() {
        let svc = GrokLLMService::new("xai_key", "").with_top_p(0.85);
        let req = svc.build_request();
        assert_eq!(req.top_p, Some(0.85));
    }

    #[test]
    fn test_build_request_with_frequency_penalty() {
        let svc = GrokLLMService::new("xai_key", "").with_frequency_penalty(0.6);
        let req = svc.build_request();
        assert_eq!(req.frequency_penalty, Some(0.6));
    }

    #[test]
    fn test_build_request_with_presence_penalty() {
        let svc = GrokLLMService::new("xai_key", "").with_presence_penalty(0.4);
        let req = svc.build_request();
    ...
    fn test_build_request_with_messages() {
        let mut svc = GrokLLMService::new("xai_key", "");
    ...
    fn test_build_request_serialization() {
        ...
        assert!(json.contains("\"model\":\"grok-2\""));
    ...
    #[test]
    fn test_build_request_with_all_sampling_params() {
        let svc = GrokLLMService::new("xai_key", "grok-2")
            .with_temperature(0.7)
            .with_top_p(0.9)
    ...
    fn test_parse_content_chunk() {
        let json = r#"{"id":"chatcmpl-123","model":"grok-2","choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}]}"#;
    ...
    fn test_parse_chunk_with_model_field() {
        let json = r#"{"id":"chatcmpl-model","model":"grok-2-mini","choices":[{"index":0,"delta":{"content":"hi"},"finish_reason":null}]}"#;
        let chunk: ChatCompletionChunk = serde_json::from_str(json).expect("parse should succeed");
        assert_eq!(chunk.model.as_deref(), Some("grok-2-mini"));
    }
    ...
    fn test_usage_metrics_data_construction() {
        let usage_metrics = LLMUsageMetricsData {
            processor: "GrokLLMService(grok-2)".to_string(),
    ...
    #[test]
    fn test_parse_empty_string_json() {
        let json = r#""""#;
        let result: Result<ChatCompletionChunk, _> = serde_json::from_str(json);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_model_grok_beta() {
        let svc = GrokLLMService::new("xai_key", "grok-beta");
        assert_eq!(svc.model, "grok-beta");
    }
    ...
    fn test_debug_format() {
        ...
        assert!(debug_str.contains("GrokLLMService"));
        assert!(debug_str.contains("grok-2"));
        assert!(debug_str.contains("api.x.ai"));
    }
    ...
    fn test_display_format() {
        ...
        assert!(display_str.contains("GrokLLMService"));
    }
    ...
    fn test_model_trait_method() {
        let svc = GrokLLMService::new("xai_key", "grok-beta");
        assert_eq!(svc.model(), Some("grok-beta"));
    }
    ...
    fn test_base_accessor() {
        let svc = GrokLLMService::new("xai_key", "");
        let base = svc.base();
        assert!(base.name().contains("GrokLLMService"));
    }
    ...
    fn test_streaming_url_construction() {
        ...
        assert_eq!(expected, "https://api.x.ai/v1/chat/completions");

        // With custom base URL
        let svc2 = GrokLLMService::new("xai_key", "").with_base_url("https://proxy.test/api");
    ...
    fn test_very_long_model_name() {
        ...
        let svc = GrokLLMService::new("xai_key", long_name.clone());
        assert_eq!(svc.model, long_name);
    }

    #[test]
    fn test_temperature_boundary_zero() {
        let svc = GrokLLMService::new("xai_key", "").with_temperature(0.0);
        assert_eq!(svc.temperature, Some(0.0));
    }

    #[test]
    fn test_temperature_boundary_two() {
        let svc = GrokLLMService::new("xai_key", "").with_temperature(2.0);
        assert_eq!(svc.temperature, Some(2.0));
    }

    #[test]
    fn test_max_tokens_boundary_one() {
        let svc = GrokLLMService::new("xai_key", "").with_max_tokens(1);
        assert_eq!(svc.max_tokens, Some(1));
    }

    #[test]
    fn test_max_tokens_large_value() {
        let svc = GrokLLMService::new("xai_key", "").with_max_tokens(131072);
    ...
    fn test_parse_chunk_extra_fields_ignored() {
        // Grok may return fields not in our struct; serde should ignore them.
    ...
    fn test_top_p_boundary_zero() {
        let svc = GrokLLMService::new("xai_key", "").with_top_p(0.0);
        assert_eq!(svc.top_p, Some(0.0));
    }

    #[test]
    fn test_top_p_boundary_one() {
        let svc = GrokLLMService::new("xai_key", "").with_top_p(1.0);
        assert_eq!(svc.top_p, Some(1.0));
    }

    #[test]
    fn test_frequency_penalty_negative() {
        let svc = GrokLLMService::new("xai_key", "").with_frequency_penalty(-2.0);
        assert_eq!(svc.frequency_penalty, Some(-2.0));
    }

    #[test]
    fn test_presence_penalty_negative() {
        let svc = GrokLLMService::new("xai_key", "").with_presence_penalty(-1.5);
        assert_eq!(svc.presence_penalty, Some(-1.5));
    }
}
