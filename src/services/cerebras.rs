...
//! Cerebras service implementation for the Pipecat Rust framework.
//!
//! This module provides:
//!
//! - [`CerebrasLLMService`] -- streaming chat-completion LLM service that talks
//!   to the Cerebras `/v1/chat/completions` endpoint.
//!
//! Cerebras offers an OpenAI-compatible API with extremely fast inference
//! powered by their wafer-scale chips. Because the wire format is identical to
...
// ---------------------------------------------------------------------------
// Cerebras API request / response types (OpenAI-compatible subset for streaming)
// ---------------------------------------------------------------------------

/// Body sent to Cerebras's `/v1/chat/completions`.
...
/// streaming inference call against the Cerebras API, emitting
...
///
/// ```ignore
/// use pipecat::services::cerebras::CerebrasLLMService;
///
/// let service = CerebrasLLMService::new("csk_abc123", "")
///     .with_temperature(0.7)
///     .with_max_tokens(1024);
/// ```
pub struct CerebrasLLMService {
    base: BaseProcessor,
    api_key: String,
    model: String,
    base_url: String,
...
impl CerebrasLLMService {
    /// Default model used when none is specified.
    pub const DEFAULT_MODEL: &'static str = "llama-3.3-70b";

    /// Default base URL for the Cerebras API.
    pub const DEFAULT_BASE_URL: &'static str = "https://api.cerebras.ai/v1";

    /// Create a new `CerebrasLLMService`.
    ///
    /// # Arguments
    ///
    /// * `api_key` -- Cerebras API key.
    /// * `model` -- Model identifier (e.g. `"llama-3.3-70b"`). Pass an empty
    ...
    pub fn new(api_key: impl Into<String>, model: impl Into<String>) -> Self {
        ...
        Self {
            base: BaseProcessor::new(Some(format!("CerebrasLLMService({})", model)), false),
            api_key,
            model,
    ...
    async fn process_streaming_response(&mut self) {
        ...
        debug!(
            ...
            "Starting streaming chat completion via Cerebras"
        );
            ...
            .await
        {
            Ok(resp) => resp,
            Err(e) => {
                error!(error = %e, "Failed to send Cerebras chat completion request");
                let err_frame =
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %error_body, "Cerebras API returned an error");
            let err_frame = Arc::new(ErrorFrame::new(
                format!("Cerebras API error (HTTP {status}): {error_body}"),
                false,
            ));
            self.base
                .pending_frames
        ...
        // --- Parse SSE stream ----------------------------------------------------
        //
        // Cerebras uses the same SSE format as OpenAI:
        //
        //   data: {"id":"...","choices":[...]}\n\n
        //
        ...
        while let Some(chunk_result) = byte_stream.next().await {
            let chunk = match chunk_result {
                Ok(c) => c,
                Err(e) => {
                    error!(error = %e, "Error reading Cerebras SSE stream");
                    let err_frame = Arc::new(ErrorFrame::new(
            ...
            while let Some(newline_pos) = line_buffer.find('\n') {
                ...
                if data == "[DONE]" {
                    debug!("Cerebras SSE stream completed");
                    break;
                }
                ...
                let chunk: ChatCompletionChunk = match serde_json::from_str(data) {
                    Ok(c) => c,
                    Err(e) => {
                        warn!(error = %e, data = %data, "Failed to parse Cerebras SSE chunk JSON");
                        continue;
                    }
                };
...
impl fmt::Debug for CerebrasLLMService {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CerebrasLLMService")
...
impl_base_display!(CerebrasLLMService);
...
#[async_trait]
impl FrameProcessor for CerebrasLLMService {
    ...
    async fn process_frame(&mut self, frame: Arc<dyn Frame>, direction: FrameDirection) {
        ...
        if let Some(append) = frame.as_any().downcast_ref::<LLMMessagesAppendFrame>() {
            ...
            debug!(
                ...
                "Appended messages, starting Cerebras inference"
            );
        ...
        if let Some(result_frame) = frame.as_any().downcast_ref::<FunctionCallResultFrame>() {
            ...
            debug!(
                ...
                "Function call result received, re-running Cerebras inference"
            );
            self.process_streaming_response().await;
            return;
        }
...
#[async_trait]
impl AIService for CerebrasLLMService {
    fn model(&self) -> Option<&str> {
        Some(&self.model)
    }

    async fn start(&mut self) {
        debug!(model = %self.model, "CerebrasLLMService started");
    }

    async fn stop(&mut self) {
        debug!("CerebrasLLMService stopped");
        self.messages.clear();
    }

    async fn cancel(&mut self) {
        debug!("CerebrasLLMService cancelled");
    }
}
...
#[async_trait]
impl LLMService for CerebrasLLMService {
    ...
    async fn run_inference(&mut self, messages: &[serde_json::Value]) -> Option<String> {
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %body_text, "Cerebras run_inference API error");
            return None;
        }
        ...
        let parsed: ChatCompletionResponse = match response.json().await {
            Ok(p) => p,
            Err(e) => {
                error!(error = %e, "Failed to parse Cerebras run_inference response");
                return None;
            }
        };
...
mod tests {
    ...
    fn test_new_with_default_model() {
        let svc = CerebrasLLMService::new("csk_test_key", "");
        assert_eq!(svc.model, CerebrasLLMService::DEFAULT_MODEL);
        assert_eq!(svc.api_key, "csk_test_key");
        assert_eq!(svc.base_url, CerebrasLLMService::DEFAULT_BASE_URL);
    }

    #[test]
    fn test_new_with_custom_model() {
        let svc = CerebrasLLMService::new("csk_key", "llama-3.1-8b");
        assert_eq!(svc.model, "llama-3.1-8b");
    }

    #[test]
    fn test_with_model_builder() {
        let svc = CerebrasLLMService::new("csk_key", "").with_model("llama-3.1-70b");
        assert_eq!(svc.model, "llama-3.1-70b");
    }

    #[test]
    fn test_with_base_url_builder() {
        let svc = CerebrasLLMService::new("csk_key", "")
    ...
    fn test_with_temperature_builder() {
        let svc = CerebrasLLMService::new("csk_key", "").with_temperature(0.7);
        assert_eq!(svc.temperature, Some(0.7));
    }

    #[test]
    fn test_with_max_tokens_builder() {
        let svc = CerebrasLLMService::new("csk_key", "").with_max_tokens(2048);
    ...
    fn test_default_base_url_constant() {
        assert_eq!(
            CerebrasLLMService::DEFAULT_BASE_URL,
    ...
    fn test_default_model_constant() {
        assert_eq!(CerebrasLLMService::DEFAULT_MODEL, "llama-3.3-70b");
    }
    ...
    fn test_build_request_basic() {
        let svc = CerebrasLLMService::new("csk_key", "llama-3.3-70b");
    ...
    fn test_build_request_with_temperature() {
        let svc = CerebrasLLMService::new("csk_key", "").with_temperature(0.3);
        let req = svc.build_request();
    ...
    fn test_build_request_with_max_tokens() {
        let svc = CerebrasLLMService::new("csk_key", "").with_max_tokens(4096);
        let req = svc.build_request();
    ...
    fn test_build_request_with_messages() {
        let mut svc = CerebrasLLMService::new("csk_key", "");
    ...
    fn test_build_request_serialization() {
        ...
        assert!(json.contains("\"model\":\"llama-3.3-70b\""));
    ...
    fn test_parse_content_chunk() {
        let json = r#"{"id":"chatcmpl-123","model":"llama-3.3-70b","choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}]}"#;
    ...
    fn test_parse_chunk_with_model_field() {
        let json = r#"{"id":"chatcmpl-model","model":"llama-3.1-8b","choices":[{"index":0,"delta":{"content":"hi"},"finish_reason":null}]}"#;
        let chunk: ChatCompletionChunk = serde_json::from_str(json).expect("parse should succeed");
        assert_eq!(chunk.model.as_deref(), Some("llama-3.1-8b"));
    }
    ...
    fn test_usage_metrics_data_construction() {
        let usage_metrics = LLMUsageMetricsData {
            processor: "CerebrasLLMService(llama-3.3-70b)".to_string(),
    ...
    fn test_model_llama_3_1_70b() {
        let svc = CerebrasLLMService::new("csk_key", "llama-3.1-70b");
        assert_eq!(svc.model, "llama-3.1-70b");
    }
    ...
    fn test_debug_format() {
        ...
        assert!(debug_str.contains("CerebrasLLMService"));
        assert!(debug_str.contains("llama-3.3-70b"));
        assert!(debug_str.contains("api.cerebras.ai"));
    }
    ...
    fn test_display_format() {
        ...
        assert!(display_str.contains("CerebrasLLMService"));
    }
    ...
    fn test_model_trait_method() {
        let svc = CerebrasLLMService::new("csk_key", "llama-3.1-70b");
        assert_eq!(svc.model(), Some("llama-3.1-70b"));
    }
    ...
    fn test_base_accessor() {
        let svc = CerebrasLLMService::new("csk_key", "");
        let base = svc.base();
        assert!(base.name().contains("CerebrasLLMService"));
    }
    ...
    fn test_streaming_url_construction() {
        ...
        assert_eq!(expected, "https://api.cerebras.ai/v1/chat/completions");

        // With custom base URL
        let svc2 = CerebrasLLMService::new("csk_key", "").with_base_url("https://proxy.test/api");
    ...
    fn test_empty_api_key() {
        let svc = CerebrasLLMService::new("", "");
    ...
    fn test_very_long_model_name() {
        ...
        let svc = CerebrasLLMService::new("csk_key", long_name.clone());
        assert_eq!(svc.model, long_name);
    }

    #[test]
    fn test_temperature_boundary_zero() {
        let svc = CerebrasLLMService::new("csk_key", "").with_temperature(0.0);
        assert_eq!(svc.temperature, Some(0.0));
    }

    #[test]
    fn test_temperature_boundary_two() {
        let svc = CerebrasLLMService::new("csk_key", "").with_temperature(2.0);
        assert_eq!(svc.temperature, Some(2.0));
    }

    #[test]
    fn test_max_tokens_boundary_one() {
        let svc = CerebrasLLMService::new("csk_key", "").with_max_tokens(1);
        assert_eq!(svc.max_tokens, Some(1));
    }

    #[test]
    fn test_max_tokens_large_value() {
        let svc = CerebrasLLMService::new("csk_key", "").with_max_tokens(131072);
    ...
    fn test_parse_chunk_extra_fields_ignored() {
        // Cerebras may return fields not in our struct; serde should ignore them.
