...
//! Together AI service implementation for the Pipecat Rust framework.
//!
//! This module provides:
//!
//! - [`TogetherLLMService`] -- streaming chat-completion LLM service that talks
//!   to the Together AI `/v1/chat/completions` endpoint.
//!
//! Together AI is an open-source model hosting platform with an
//! OpenAI-compatible API. It hosts a wide range of open-source models including
//! Meta Llama, Mistral, Qwen, and more.
//!
//! # Supported models
//!
...
// ---------------------------------------------------------------------------
// Together AI API request / response types (OpenAI-compatible with extensions)
// ---------------------------------------------------------------------------

/// Body sent to Together AI's `/v1/chat/completions`.
...
/// streaming inference call against the Together AI API, emitting
...
///
/// ```ignore
/// use pipecat::services::together::TogetherLLMService;
///
/// let service = TogetherLLMService::new("tog_abc123", "")
...
pub struct TogetherLLMService {
    ...
    top_k: Option<u64>,
    /// Optional repetition penalty (Together AI specific, 1.0 = no penalty).
    repetition_penalty: Option<f64>,
    /// Optional stop sequences.
    stop: Option<Vec<String>>,
}

impl TogetherLLMService {
    ...
    pub const DEFAULT_BASE_URL: &'static str = "https://api.together.xyz";

    /// Create a new `TogetherLLMService`.
    ///
    /// # Arguments
    ///
    /// * `api_key` -- Together AI API key.
    /// * `model` -- Model identifier (e.g.
    ///   `"meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo"`). Pass an empty
    ...
    pub fn new(api_key: impl Into<String>, model: impl Into<String>) -> Self {
        ...
        Self {
            base: BaseProcessor::new(Some(format!("TogetherLLMService({})", model)), false),
            api_key,
            model,
    ...
    /// Builder method: set the top-p (nucleus sampling) parameter.
    pub fn with_top_p(mut self, top_p: f64) -> Self {
        self.top_p = Some(top_p);
        self
    }

    /// Builder method: set the top-k sampling parameter.
    pub fn with_top_k(mut self, top_k: u64) -> Self {
        self.top_k = Some(top_k);
        self
    }

    /// Builder method: set the repetition penalty (Together AI specific).
    ///
    /// A value of 1.0 means no penalty. Values greater than 1.0 discourage
    /// repetition; values less than 1.0 encourage it.
    pub fn with_repetition_penalty(mut self, penalty: f64) -> Self {
        self.repetition_penalty = Some(penalty);
        self
    }

    /// Builder method: set stop sequences that halt generation.
    ...
    async fn process_streaming_response(&mut self) {
        ...
        debug!(
            ...
            "Starting streaming chat completion via Together AI"
        );
            ...
            .await
        {
            Ok(resp) => resp,
            Err(e) => {
                error!(error = %e, "Failed to send Together AI chat completion request");
                let err_frame =
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %error_body, "Together AI API returned an error");
            let err_frame = Arc::new(ErrorFrame::new(
                format!("Together AI API error (HTTP {status}): {error_body}"),
                false,
            ));
            self.base
                .pending_frames
        ...
        // --- Parse SSE stream ----------------------------------------------------
        //
        // Together AI uses the same SSE format as OpenAI:
        //
        //   data: {"id":"...","choices":[...]}\n\n
        //
        ...
        while let Some(chunk_result) = byte_stream.next().await {
            let chunk = match chunk_result {
                Ok(c) => c,
                Err(e) => {
                    error!(error = %e, "Error reading Together AI SSE stream");
                    let err_frame = Arc::new(ErrorFrame::new(
            ...
            while let Some(newline_pos) = line_buffer.find('\n') {
                ...
                if data == "[DONE]" {
                    debug!("Together AI SSE stream completed");
                    break;
                }
                ...
                let chunk: ChatCompletionChunk = match serde_json::from_str(data) {
                    Ok(c) => c,
                    Err(e) => {
                        warn!(error = %e, data = %data, "Failed to parse Together AI SSE chunk JSON");
                        continue;
                    }
                };
...
impl fmt::Debug for TogetherLLMService {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TogetherLLMService")
...
impl_base_display!(TogetherLLMService);
...
#[async_trait]
impl FrameProcessor for TogetherLLMService {
    ...
    async fn process_frame(&mut self, frame: Arc<dyn Frame>, direction: FrameDirection) {
        ...
        if let Some(append) = frame.as_any().downcast_ref::<LLMMessagesAppendFrame>() {
            ...
            debug!(
                ...
                "Appended messages, starting Together AI inference"
            );
        ...
        if let Some(result_frame) = frame.as_any().downcast_ref::<FunctionCallResultFrame>() {
            ...
            debug!(
                ...
                "Function call result received, re-running Together AI inference"
            );
            self.process_streaming_response().await;
            return;
        }
...
#[async_trait]
impl AIService for TogetherLLMService {
    fn model(&self) -> Option<&str> {
        Some(&self.model)
    }

    async fn start(&mut self) {
        debug!(model = %self.model, "TogetherLLMService started");
    }

    async fn stop(&mut self) {
        debug!("TogetherLLMService stopped");
        self.messages.clear();
    }

    async fn cancel(&mut self) {
        debug!("TogetherLLMService cancelled");
    }
}
...
#[async_trait]
impl LLMService for TogetherLLMService {
    ...
    async fn run_inference(&mut self, messages: &[serde_json::Value]) -> Option<String> {
        ...
        if !response.status().is_success() {
            ...
            error!(status = %status, body = %body_text, "Together AI run_inference API error");
            return None;
        }
        ...
        let parsed: ChatCompletionResponse = match response.json().await {
            Ok(p) => p,
            Err(e) => {
                error!(error = %e, "Failed to parse Together AI run_inference response");
                return None;
            }
        };
...
mod tests {
    ...
    fn test_new_with_default_model() {
        let svc = TogetherLLMService::new("tog_test_key", "");
        assert_eq!(svc.model, TogetherLLMService::DEFAULT_MODEL);
        assert_eq!(svc.api_key, "tog_test_key");
        assert_eq!(svc.base_url, TogetherLLMService::DEFAULT_BASE_URL);
    }

    #[test]
    fn test_new_with_custom_model() {
        let svc = TogetherLLMService::new("tog_key", "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo");
        assert_eq!(svc.model, "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo");
    }

    #[test]
    fn test_with_model_builder() {
        let svc = TogetherLLMService::new("tog_key", "")
            .with_model("mistralai/Mixtral-8x7B-Instruct-v0.1");
        assert_eq!(svc.model, "mistralai/Mixtral-8x7B-Instruct-v0.1");
    }
    ...
    fn test_with_temperature_builder() {
        let svc = TogetherLLMService::new("tog_key", "").with_temperature(0.7);
        assert_eq!(svc.temperature, Some(0.7));
    }

    #[test]
    fn test_with_max_tokens_builder() {
        let svc = TogetherLLMService::new("tog_key", "").with_max_tokens(2048);
        assert_eq!(svc.max_tokens, Some(2048));
    }

    #[test]
    fn test_with_top_p_builder() {
        let svc = TogetherLLMService::new("tog_key", "").with_top_p(0.9);
        assert_eq!(svc.top_p, Some(0.9));
    }

    #[test]
    fn test_with_top_k_builder() {
        let svc = TogetherLLMService::new("tog_key", "").with_top_k(50);
        assert_eq!(svc.top_k, Some(50));
    }

    #[test]
    fn test_with_repetition_penalty_builder() {
        let svc = TogetherLLMService::new("tog_key", "").with_repetition_penalty(1.1);
        assert_eq!(svc.repetition_penalty, Some(1.1));
    }

    #[test]
    fn test_with_stop_builder() {
        let svc = TogetherLLMService::new("tog_key", "")
            .with_stop(vec!["<|eot|>".to_string(), "\n\n".to_string()]);
        assert_eq!(
            svc.stop,
    ...
    fn test_builder_chaining() {
        let svc = TogetherLLMService::new("tog_key", "")
            .with_model("Qwen/Qwen2.5-72B-Instruct-Turbo")
        ...
        assert_eq!(svc.model, "Qwen/Qwen2.5-72B-Instruct-Turbo");
        ...
        assert_eq!(svc.stop, Some(vec!["STOP".to_string()]));
    ...
    #[test]
    fn test_default_stop_is_none() {
        let svc = TogetherLLMService::new("tog_key", "");
        assert!(svc.stop.is_none());
    }
    ...
    fn test_default_base_url_constant() {
        assert_eq!(
            TogetherLLMService::DEFAULT_BASE_URL,
    ...
    fn test_default_model_constant() {
        assert_eq!(
            TogetherLLMService::DEFAULT_MODEL,
    ...
    fn test_build_request_basic() {
        let svc =
            TogetherLLMService::new("tog_key", "meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo");
    ...
    fn test_build_request_with_temperature() {
        let svc = TogetherLLMService::new("tog_key", "").with_temperature(0.3);
        let req = svc.build_request();
    ...
    fn test_build_request_with_max_tokens() {
        let svc = TogetherLLMService::new("tog_key", "").with_max_tokens(4096);
        let req = svc.build_request();
    ...
    fn test_build_request_with_top_p() {
        let svc = TogetherLLMService::new("tog_key", "").with_top_p(0.85);
        let req = svc.build_request();
    ...
    fn test_build_request_with_top_k() {
        let svc = TogetherLLMService::new("tog_key", "").with_top_k(100);
        let req = svc.build_request();
    ...
    fn test_build_request_with_repetition_penalty() {
        let svc = TogetherLLMService::new("tog_key", "").with_repetition_penalty(1.15);
        let req = svc.build_request();
        assert_eq!(req.repetition_penalty, Some(1.15));
    }

    #[test]
    fn test_build_request_with_stop_sequences() {
        let svc = TogetherLLMService::new("tog_key", "")
            .with_stop(vec!["END".to_string(), "HALT".to_string()]);
        let req = svc.build_request();
        assert_eq!(req.stop, Some(vec!["END".to_string(), "HALT".to_string()]));
    }

    #[test]
    fn test_build_request_with_messages() {
        let mut svc = TogetherLLMService::new("tog_key", "");
    ...
    fn test_build_request_serialization() {
        ...
        assert!(json.contains("\"model\":\"meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo\""));
    ...
    fn test_parse_content_chunk() {
        let json = r#"{"id":"chatcmpl-123","model":"meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo","choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}]}"#;
    ...
    fn test_parse_chunk_with_model_field() {
        let json = r#"{"id":"chatcmpl-model","model":"meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo","choices":[{"index":0,"delta":{"content":"hi"},"finish_reason":null}]}"#;
        let chunk: ChatCompletionChunk = serde_json::from_str(json).expect("parse should succeed");
        assert_eq!(
    ...
    fn test_usage_metrics_data_construction() {
        let usage_metrics = LLMUsageMetricsData {
            processor: "TogetherLLMService(meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo)"
                .to_string(),
    ...
    fn test_parse_non_streaming_response() {
        let json = r#"{
            "choices":[{
                "message":{"content":"Hello from Together AI!"}
            }],
    ...
    #[test]
    fn test_model_llama_3_1_8b() {
        let svc = TogetherLLMService::new("tog_key", "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo");
        assert_eq!(svc.model, "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo");
        let req = svc.build_request();
        assert_eq!(req.model, "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo");
    }

    #[test]
    fn test_model_mixtral() {
        let svc = TogetherLLMService::new("tog_key", "mistralai/Mixtral-8x7B-Instruct-v0.1");
        assert_eq!(svc.model, "mistralai/Mixtral-8x7B-Instruct-v0.1");
    }

    #[test]
    fn test_model_qwen() {
        let svc = TogetherLLMService::new("tog_key", "Qwen/Qwen2.5-72B-Instruct-Turbo");
        assert_eq!(svc.model, "Qwen/Qwen2.5-72B-Instruct-Turbo");
    }
    ...
    fn test_debug_format() {
        ...
        assert!(debug_str.contains("TogetherLLMService"));
        assert!(debug_str.contains("Meta-Llama-3.1-70B-Instruct-Turbo"));
        assert!(debug_str.contains("api.together.xyz"));
    }
    ...
    fn test_display_format() {
        ...
        assert!(display_str.contains("TogetherLLMService"));
    }
    ...
    fn test_model_trait_method() {
        let svc = TogetherLLMService::new("tog_key", "mistralai/Mixtral-8x7B-Instruct-v0.1");
        assert_eq!(svc.model(), Some("mistralai/Mixtral-8x7B-Instruct-v0.1"));
    }
    ...
    fn test_base_accessor() {
        let svc = TogetherLLMService::new("tog_key", "");
        let base = svc.base();
        assert!(base.name().contains("TogetherLLMService"));
    }
    ...
    fn test_streaming_url_construction() {
        let svc = TogetherLLMService::new("tog_key", "");
        let expected = format!("{}/v1/chat/completions", svc.base_url);
        assert_eq!(expected, "https://api.together.xyz/v1/chat/completions");

        // With custom base URL
        let svc2 = TogetherLLMService::new("tog_key", "").with_base_url("https://proxy.test/api");
        let url2 = format!("{}/v1/chat/completions", svc2.base_url);
        assert_eq!(url2, "https://proxy.test/api/v1/chat/completions");
    }

    #[test]
    fn test_non_streaming_url_construction() {
        let svc = TogetherLLMService::new("tog_key", "");
        let url = format!("{}/v1/chat/completions", svc.base_url);
    ...
    fn test_repetition_penalty_no_penalty() {
        let svc = TogetherLLMService::new("tog_key", "").with_repetition_penalty(1.0);
    ...
    #[test]
    fn test_repetition_penalty_high_value() {
        let svc = TogetherLLMService::new("tog_key", "").with_repetition_penalty(2.0);
        assert_eq!(svc.repetition_penalty, Some(2.0));
    }

    #[test]
    fn test_repetition_penalty_low_value() {
        let svc = TogetherLLMService::new("tog_key", "").with_repetition_penalty(0.5);
        assert_eq!(svc.repetition_penalty, Some(0.5));
    }

    #[test]
    fn test_repetition_penalty_in_serialized_request() {
        let svc = TogetherLLMService::new("tog_key", "").with_repetition_penalty(1.3);
        let req = svc.build_request();
        let json = serde_json::to_string(&req).expect("serialization should succeed");
        assert!(json.contains("\"repetition_penalty\":1.3"));
    }
    ...
    fn test_empty_api_key() {
        let svc = TogetherLLMService::new("", "");
    ...
    fn test_very_long_model_name() {
        ...
        let svc = TogetherLLMService::new("tog_key", long_name.clone());
        assert_eq!(svc.model, long_name);
    }

    #[test]
    fn test_temperature_boundary_zero() {
        let svc = TogetherLLMService::new("tog_key", "").with_temperature(0.0);
        assert_eq!(svc.temperature, Some(0.0));
    }

    #[test]
    fn test_temperature_boundary_two() {
        let svc = TogetherLLMService::new("tog_key", "").with_temperature(2.0);
        assert_eq!(svc.temperature, Some(2.0));
    }

    #[test]
    fn test_max_tokens_boundary_one() {
        let svc = TogetherLLMService::new("tog_key", "").with_max_tokens(1);
        assert_eq!(svc.max_tokens, Some(1));
    }

    #[test]
    fn test_max_tokens_large_value() {
        let svc = TogetherLLMService::new("tog_key", "").with_max_tokens(131072);
    ...
    fn test_parse_chunk_extra_fields_ignored() {
        // Together AI may return fields not in our struct; serde should ignore them.
    ...
    fn test_top_k_boundary_one() {
        let svc = TogetherLLMService::new("tog_key", "").with_top_k(1);
        assert_eq!(svc.top_k, Some(1));
    }

    #[test]
    fn test_top_p_boundary_zero() {
        let svc = TogetherLLMService::new("tog_key", "").with_top_p(0.0);
        assert_eq!(svc.top_p, Some(0.0));
    }

    #[test]
    fn test_top_p_boundary_one() {
        let svc = TogetherLLMService::new("tog_key", "").with_top_p(1.0);
        assert_eq!(svc.top_p, Some(1.0));
    }

    #[test]
    fn test_stop_empty_vec() {
        let svc = TogetherLLMService::new("tog_key", "").with_stop(vec![]);
        assert_eq!(svc.stop, Some(vec![]));
    }

    #[test]
    fn test_stop_in_serialized_request() {
        let svc = TogetherLLMService::new("tog_key", "").with_stop(vec!["<|end|>".to_string()]);
        let req = svc.build_request();
        let json = serde_json::to_string(&req).expect("serialization should succeed");
