...
impl FrameEnum {
    /// Returns a `&dyn Frame` reference to the inner concrete frame struct.
    ///
    /// This enables `downcast_ref` on the concrete type (e.g. `TextFrame`),
    /// which is useful for serializers that have not yet been migrated to
    /// `FrameEnum` pattern matching.
    ///
